...
miniRPC 的压测目标不是只报告最高 QPS，而是用可复现的固定场景比较两条服务端路径：
...
| `total_responses` | 成功进入发送队列或完成写出的响应总数，包含成功、错误、超时和拒绝 | Reactor / Coroutine 已记录 |
...
| `pending_requests` | 等待 handler 或等待响应的请求数 | 已记录 |
| `avg_latency` | 服务端全生命周期累计平均处理延迟 | 已记录 |
| `p50/p95/p99_latency` | 最近最多 10000 条延迟样本排序计算的分位数 | 已记录 |
...
```text
实现 Reactor + ThreadPool 与 Coroutine + epoll + ThreadPool 两条服务端路径，
设计同进程闭环压测，在固定 Echo、64 连接和慢 handler 场景下各运行 5 轮，
用中位数比较 QPS 与 P50/P95/P99，并汇总全部轮次的失败和状态码以避免隐藏偶发错误。
```
